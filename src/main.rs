    ...
    loop {
        match try_join!(jupiter_swap_api_client.quote(&sell_request), jupiter_swap_api_client.quote(&buy_request)) {
            Ok((sell_response, buy_response)) => {
                ...
                let print_buy_price = buy_price * 2.0;
                let print_sell_price = sell_price * 2.0;

                println!("{timestamp:?}, {act_usdc:.2}, {current_buy_rsi:.2}, {print_buy_price:.3}, {act_sol:.3}, {current_sell_rsi:.2}, {print_sell_price:.2}, {buy_sell_flag}, {total:.2}");
